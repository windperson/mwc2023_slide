...
6. 用 Visual Studio 2022 專案精靈建立 Blazor WASM 專案及配套來跑的 asp.net core 測試後端專案
7. 將 RCL 專案加入到 Blazor WASM 專案的專案參考 (Project Reference)
8. 搬移 Blazor WASM 專案與 Razor 頁面相關之目錄＆檔案，覆蓋掉與 RCL 專案中既有的 CSS/js 檔案
9. 修改還留在 Blazor WASM 專案中的必要 Blazor 所需檔案內容，包括網頁靜態資源引用路徑、Blazor 路由元件增加 `AdditionalAssemblies` 屬性設定
